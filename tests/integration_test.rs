...
use common::{CountingCallbacks, TestRepo, init_repo, setup_workspace_with_repos, test_config};
...
fn test_workspace_repos_on_different_branches() -> anyhow::Result<()> {
    ...
    assert_eq!(
        git::get_current_branch(&master_repo, &config, logger())?,
        "feature-x"
    );
    assert_eq!(
        git::get_current_branch(&main_repo, &config, logger())?,
        "develop"
    );
        ...
        match &result.outcome {
            UpdateOutcome::Success(s) => {
                if result.path.ends_with("repo-on-master") {
                    assert_eq!(
                        s.original_head,
                        OriginalHead::Branch("feature-x".to_string())
                    );
...
fn test_update_handles_detached_head() -> anyhow::Result<()> {
    ...
    // Verify we're in a detached HEAD state
    let branch = git::get_current_branch(repo.path(), &config, logger())?;
...
fn test_delete_branch_removes_branch() -> anyhow::Result<()> {
    ...
    let repo = TestRepo::new()?;
    repo.create_branch("feature")?;
    let output_before = git::run_git(repo.path(), &config, &["branch", "--list", "feature"])?;
    assert!(!output_before.trim().is_empty());
